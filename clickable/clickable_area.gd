...
func _is_usable() -> bool: 
	if _view_dependency and _event_dependency:
		if !_view_dependency.is_focused():
			return false
		elif !_event_dependency.is_triggered():
			return false
		return true
	elif _view_dependency:
		return _view_dependency.is_focused()
	elif _event_dependency:
		return _event_dependency.is_triggered()
	return true
